...
# Almacenar los landmarks y las distancias en listas
lista_landmarks = [list(landmark) for landmark in landmarks]
lista_dist_ojo_der = [list(dist) for dist in dist_ojo_der]
lista_dist_ojo_izq = [list(dist) for dist in dist_ojo_izq]
lista_dist_ceja_der = [list(dist) for dist in dist_ceja_der]
lista_dist_ceja_izq = [list(dist) for dist in dist_ceja_izq]
lista_dist_nariz = [list(dist) for dist in dist_nariz]
lista_dist_forma = [list(dist) for dist in dist_forma]

response_data_list = []
for i in range(len(lista_landmarks)):
    response_data = {
        "landmarks": lista_landmarks[i],
        "dist_ojo_der": lista_dist_ojo_der[i],
        "dist_ojo_izq": lista_dist_ojo_izq[i],
        "dist_ceja_der": lista_dist_ceja_der[i],
        "dist_ceja_izq": lista_dist_ceja_izq[i],
        "dist_nariz": lista_dist_nariz[i],
        "dist_forma": lista_dist_forma[i]
    }
